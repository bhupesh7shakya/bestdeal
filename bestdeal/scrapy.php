...
$getsSearchQuery = $_GET['q'];
   ...
   if ($result) {
      if (mysqli_num_rows($result) != 0) {
         ...
         foreach ($html->find('div.products') as $product) {
            ...
            $price = $product->find('div[class=product_price text-right pull-right]', 0)->plaintext;
         ...
         while ($i < $num) {
            ...
            if ($result) {
               // echo 'inserted<br>';
               mysqli_query($con, "DELETE FROM `products` WHERE `products`.`product_img_url` = ''");
            } else {
               echo 'wrong query<br>';
            }
         }
   ...
   mysqli_query($con, "DELETE FROM `products` WHERE `products`.`product_price` = ''");
